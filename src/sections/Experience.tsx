    ...
    >
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 w-px bg-[rgba(245,247,255,0.14)] -translate-x-1/2" />
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden lg:flex absolute left-1/2 top-0 -translate-x-1/2 z-10">
                    <div className="w-4 h-4 rounded-full accent-bg border-4 border-secondary-dark" />
                  </div>
                  ...
                  >
                    <div className="p-6 bg-navy-800/50 card-border hover:border-indigo-accent/40 transition-colors">
                      {/* Header */}
                      <div className={`flex flex-col ${isLeft ? 'lg:items-end' : 'lg:items-start'} mb-4`}>
                        <div className={`flex items-center gap-2 mb-2 ${isLeft ? 'lg:flex-row-reverse' : ''}`}>
                          <Briefcase className="w-4 h-4 text-indigo-accent" />
                          <span className="label-mono">{exp.company}</span>
                        </div>
                        <h3 className="font-display text-xl font-semibold text-primary-light mb-2">
                          {exp.role}
                        </h3>
                        <div className={`flex flex-wrap items-center gap-3 text-sm text-slate-light ${isLeft ? 'lg:justify-end' : ''}`}>
                      ...
                      {hasDetails && (
                        <ul className={`space-y-2 mb-4 ${isLeft ? 'lg:text-right' : ''}`}>
                          {exp.achievements.map((achievement, i) => (
                            <li 
                              key={i} 
                              className={`flex items-start gap-2 text-sm text-secondary-light ${isLeft ? 'lg:flex-row-reverse' : ''}`}
                            >
                              <ChevronRight className="w-4 h-4 text-indigo-accent flex-shrink-0 mt-0.5" />

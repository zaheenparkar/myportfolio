              ...
              >
                ...
                {index < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-12 left-full w-full h-px">
                    <div className="w-full h-full bg-gradient-to-r from-indigo-accent/50 to-transparent" />
                    <ChevronRight className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-accent/50" />
                  </div>
                )}

                <div className="relative h-full p-6 bg-navy-800/50 card-border hover:border-indigo-accent/40 transition-all duration-300 group-hover:-translate-y-1">
                  {/* Step Number */}
                  <div className="absolute -top-3 -left-1">
                    <span className="font-display text-5xl font-bold text-indigo-accent/20">
                      {step.number}
                    </span>
                  </div>

                  {/* Icon */}
                  <div className={`w-14 h-14 rounded-sm ${step.color} flex items-center justify-center mb-4`}>
                  ...
                  <ul className="space-y-1.5">
                    {step.details.map((detail, i) => (
                      <li key={i} className="flex items-center gap-2 text-xs text-slate-light">
                        <div className="w-1 h-1 rounded-full bg-indigo-accent" />
                        {detail}
                      </li>
                    ))}
                  </ul>
